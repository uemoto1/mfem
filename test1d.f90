...
contains
  ...
  subroutine prepare_ilu0(a, lu)
    implicit none
    type(sparse_matrix_csr), intent(in) :: a
    type(sparse_matrix_csr), intent(inout) :: lu
    integer :: i, ij_min, ij_max, ij, j, k_max, ik, k
    real(8) :: temp, l_ii
    ! generate preconditioning matrix by imcomplete lu (ilu0) decomposition
    do i = 1, lu%ndim
      ij_min = lu%irptr(i)
